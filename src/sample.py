...
# #   seq, seconds, kv, set_val, get_val, create_moment
#
# rules = RuleSet()
#
# rules.add_rule(
#     topic_is('/device/status') and 'HUB disconnect' in msg.data,
#     create_moment('hub disconnect', ts))
#
# rules.add_rule(
#     topic_is('/checker/warning_event'),
#     create_moment(evaluate(msg, lambda m: ERROR_CODES.get(str(m.error_code))), ts))
#
# rules.add_rule(
#     seq(
#         topic_is('rosout') and 'task/pause' in msg.msg,
#         topic_is('rosout') and 'start_cross_task' in msg.msg,
#         within=seconds(10)),
...
# rules.add_rule(
#     not seq(
#         topic_is('device_status') and charger_voltage > 0.0 and charger_current == 0,
#         topic_is('device_status') and charger_current > 0.0 and charger_current > 0,
#         within=seconds(20)),
#     create_moment('对桩失败问题：机器对桩后不充电 (等了20秒)'))
#
# rules.add_rule(
#     seq(
#         topic_is('request') and set_val('req_id', msg.id),
#         topic_is('reponse') and msg.req_id == get_val('req_id'),
#         within=seconds(10)),
